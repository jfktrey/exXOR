...
################################################################################################# exXOR
        thisInstructionIsString = False
        if token_t == "OP_LDC1" or token_t == "OP_LDC2":
            possibleString = poolm.pool[args[0]]
            if possibleString[0] == "String":
                thisInstructionIsString = True
                logMachine.logString(poolm.cparg1(possibleString[1][0]))
#/############################################################################################### /exXOR
        if token_t == 'OP_LBL':
            assert(len(args) == 1)
        ...
        toReturn = '\t' + ' '.join(parts)
################################################################################################# exXOR
        if logMachine.inZFunction:
            logMachine.logZFunctionLine(toReturn)
        elif logMachine.justLoggedString and not thisInstructionIsString:
            logMachine.encryptedOrPlain(toReturn)

        if thisInstructionIsString:
            toReturn = "\t;exXOR %d,%s\n%s" %(logMachine.currentStringIndex(), name, toReturn)
#/############################################################################################### /exXOR
        return toReturn
...
def disMethod(method, add, poolm):
    ...
    methodText = '.method {} {} : {}'.format(mflags, poolm.utfref(method.name_id), poolm.utfref(method.desc_id))    # exXOR
    logMachine.beginMethod(methodText)                                                                              # exXOR
    add(methodText)
    ...
    logMachine.endMethod(methodText)                                                                                # exXOR
    add('.end method')
...
def disassemble(cls, passedLogMachine):
    global logMachine               # exXOR
    logMachine = passedLogMachine   # exXOR
